...
public class Duplicator implements PlugIn, TextListener {
	...
	public void run(String arg) {
		...
		if (imp2.getWindow() != null) {
			Point p = imp.getWindow().getLocationOnScreen(); // lo: 2 lignes ajoutées pour décaler la nouvelle image
			imp2.getWindow().setLocation(p.x + imp.getWindow().getWidth() , p.y);  // exactement à droite de sa source
		}
