 ...
 * Laplacien_.java 
 *
 * Dï¿½tection de points contours par approches du second ordre.
 * Fichier ï¿½tudiant ï¿½ complï¿½ter
 *
 * @author 
 *
 */
...
public class TESTPlugin_ implements PlugInFilter, DialogListener {
	...
	// Mï¿½thode principale du plugin
	public void run(ImageProcessor ip) {

		// Affichage de la fenï¿½tre de configuration
		if (!showDialog())
			return;
		...
		// Gï¿½nï¿½ration d'un masque LoG de taille impaire (exercice 3)
		/* ï¿½ complï¿½ter */


		// Calcul et reprï¿½sentation du Laplacien (exercice 1, puis ï¿½ modifier dans le 3) 
	...
	// ---------------------------------------------------------------------------------
	/**
	 * Mï¿½thode d'affichage de la fenï¿½tre de config et de lecture des valeurs saisies, appelï¿½e dans run()
	 * 
	 * @return false si la fenï¿½tre a ï¿½tï¿½ fermï¿½e en cliquant sur Cancel, true sinon (boolean)
	 */ 

	public boolean showDialog() {

		// Description de la fenï¿½tre de config
		...
		gd.getComponent(gd.getComponentCount()-1).setEnabled(seuillageZeroCross && !seuilZeroCrossAuto); // Dï¿½sactiver champ ?
		gd.addCheckbox ("Auto threshold", seuilZeroCrossAuto);
		gd.getComponent(gd.getComponentCount()-1).setEnabled(seuillageZeroCross && seuilZeroCrossAuto); // Dï¿½sactiver case ?
	...
	// ---------------------------------------------------------------------------------
	/**
	 * Mï¿½thode appelï¿½e sur modification (par un ï¿½vï¿½nement) de la fenï¿½tre de config
	 * 
	 * @param gd Fenï¿½tre de dialogue (GenericDialog)
	 * @param e ï¿½vï¿½nement ï¿½ traiter (AWTEvent) 
	 * @return true si la saisie est correcte, false sinon (boolean)
	 */

	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {

		// Accï¿½s aux champs de la fenï¿½tre
		...
		if (e!=null) {	// e==null si clic sur OK
			// Dï¿½sactivation/activation du champ numï¿½rique de seuil, selon seuillage ou pas
	...
	/**
	 * Dï¿½tection des passages par 0 du Laplacien (algo min<-seuil && max>seuil)
	 * 
	...
	/**
	 * Gï¿½nï¿½ration d'un masque LoG dans un tableau float[]
	 * 
	 * @param tailleMasque nombre de lignes et de colonnes du masque LoG (int, impair).
	 * @param sigma ï¿½cart-type de la gaussienne (float, au moins 5*sigma)
	 * @return masque de l'opï¿½rateur LoG
	 */
...
public class TESTPlugin_ implements PlugInFilter, DialogListener {
	...
	public void run(ImageProcessor ip) {
		...
		ImagePlus image = new ImagePlus("Resultat du calcul de Laplacien", fpLaplacian);
		image.show();
		
		// Détection et affichage des passages par 0 du Laplacien par seuillage du Laplacien (exercice 2)
		if (seuillageZeroCross) {
			ImagePlus image2 = new ImagePlus("Resultat du seuillage manuel", this.laplacienZero(fpLaplacian, (float)15));
			image2.show();
	...
	public ByteProcessor laplacienZero(ImageProcessor imLaplacien, Float seuil) {
		...
		for (int x = 1; x<width; x++){
			for (int y = 1; y<height; y++){
				...
				float current = imLaplacien.getPixelValue(x,y);
				float min = current;
				float max = current;
				
				for (int i = -1; i<=1; i++){
					for (int k = -1; k<=1; k++){
						float currentNeighbour = imLaplacien.getPixelValue(x + i,y+ k);
						
						if (currentNeighbour<min)
							min = currentNeighbour;
						if (currentNeighbour>max)
							max = currentNeighbour;
//						
//						min = Math.min(imLaplacien.getPixelValue(x + i,y+ k), min);
//						max = Math.max(imLaplacien.getPixelValue(x + i,y+ k),max);
						
					}
				}
				//Math.abs(max-min)>seuil
...
>>>>>>> ea368db8cb195c501b87eaace8db05370b857f5e
}