...
void CatmullRomCurve::draw() {
    ...
    // - intermediate(i,0) et intermediate(i,1) vous donnent les deux points intermédiaires pour la courbe i (définie entre point(i) et point(i+1))
    ...
    if (nbPoint()<2) return; // on déclenche le calcul uniquement si plus de 2 points saisis.
...
void CatmullRomCurve::setup() {
    ...
    // les points intermédiaires doivent être fixés par la méthode vue en cours (parallèle au segment [point(i-1),point(i+1)]).
    if(nbPoint()>3){
    //cout<< nbPoint() <<endl;
        for(int i=1;i<nbPoint()-1;i++){
            parallele=(point(i+1)-point(i-1));
            parallele=parallele/4;
      //      cout <<parallele<<"  " <<point(i+1)<<"  " << point(i-1)<<"  " << (point(i+1)-point(i-1))/4 <<endl;
