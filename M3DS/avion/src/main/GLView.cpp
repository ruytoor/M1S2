...
/* Question  12*/
    Vector3 vtmp(0,0,3);
    Vector3 vQuar(0,1,0);
    Vector3 v=_avion.position();
    Quaternion o=_avion.orientation();
    o.rotate(180,vQuar);
    o.transform(&vtmp);
    v=v+vtmp;
    ...
    /*Question 13*/
    _camera.position((_cam1.position()*_kInterpolate)+(_cam2.position()*(1-_kInterpolate)));
    _camera.orientation((_cam1.orientation()*_kInterpolate)+(_cam2.orientation()*(1-_kInterpolate)));
...
void GLView::paintGL() {
    ...
    //v=_avion.position();
    _camera.applyGL();
    //gluLookAt(0,0,-5,v.x(),v.y(),v.z(),0,1,0);
    _trackball.applyGL();
